...
```mermaid
flowchart TD
    G["ゴール: 情報の価値 → 人への関心 → 顔と名前"] --> N["北極星: 保存率・完走率・コメント率・プロフィールアクセス率"]
    N --> K1["完走率<br/>台本の型が効いているか"]
    N --> K2["保存率<br/>情報の価値。情報系の本丸"]
    N --> K3["コメント率<br/>人格への関心の芽"]
    N --> K4["プロフィールアクセス率<br/>「この人が気になる」= 顔出し判断の分母"]
    K1 --> S1["冒頭3秒の設計"]
    K1 --> S2["2〜3秒に1回の画変化"]
    K2 --> S3["一次情報（シーン4）"]
    K3 --> S4["一人称・問いかけ"]
    K4 --> S5["ジャンルの一貫性・プロフィール文"]
```
...
|---|---|---|
| **完走率** | 台本の型が効いているか（**ジャンルではなく作りの問題**） | Phase 1（型の検証） |
| **保存率** | 情報として価値があるか。**情報系ショートの本丸** | Phase 1（ジャンルの検証） |
| **コメント率** | 人格への関心が芽生えているか | Phase 1→2 の移行判定 |
| **プロフィールアクセス率** | 「この人が気になる」 | **Phase 3（顔出し）の分母** |

**完走率が低いのはジャンルのせいではない**ことに注意。
完走率は作りの問題なので、これが全ジャンルで低ければ**台本か画変化を疑う**
（[03](./03_トレンド調査.md) 1章）。ジャンルの優劣は**保存率とコメント率**で見る。
...
集計は `tools/weekly_report.py` が**ジャンル別**に出す（[08](./08_自動化.md) 4章）。

### 判定の順番

```
1. 全ジャンルで完走率が低い？  → ジャンルの問題ではない。台本・画変化を直して測り直す
2. ジャンル間で保存率に差がある？ → その差がジャンルの実力
3. コメント率が高いジャンルは？  → Phase 2への移行可能性が高い
4. 迷ったら延長する
```

### 判定時に必ず思い出すこと

> ⚠️ **`trivia` が勝っても、そのまま採用しない。**
> ① 人格に移行しない ② C・D類型と区別がつかず2026年の規制側に立つ
> （[02](./02_競合分析.md) 3章）。数字が出た場合は「拡散の上限が分かった」と解釈する。
>
> ⚠️ **本数が少ないうちに決めない。** 1ジャンル4本では偶然が支配する。
> 僅差なら2週間延長する。**早く決めることより、間違った型に固定されないことが大事**。

### 絶対値の目標は置かない（いまは）

ジャンル別のベンチマークが公開されていない（[01](./01_市場分析.md) 8章）ため、
「保存率◯%を目指す」は根拠のない数字になる。

**Phase 1で測るのは絶対値ではなくジャンル間の相対差**。
...
- **ジャンル（`money` / `relationship` / `trivia`）別に集計する**。ここが判定の本体
...
4. **制度変更の確認**（`money` の過去動画が誤情報になっていないか。[03](./03_トレンド調査.md) 4章）
...
| **AI量産判定** | 一次情報を必ず乗せる（[04](./04_戦略.md) 2章）。TikTokでAIラベルを付ける。Phase 2で本人の声へ |
| 炎上 | 他クリエイター比較・専門家を貶める煽りをしない。公開データは自分の数値のみ |
| 本業への影響 | 会社名を出さない（Phase 2まで）。実名は**取り消せない**ことを前提に判断する |
| 継続断絶 | パイプラインがあるので「作れない」では止まらない。**止まるとしたら事実確認の工数**。省力運転は `trivia` ではなく**本数を減らす**で対応する |
