...
```mermaid
flowchart TD
    G["ゴール: ファン獲得（Phase 1-2）"] --> N["北極星: 保存率 と 完走率"]
...
| **最重要** | **3ピラーの優劣がデータで語れる状態になること** | 週次レビューの蓄積 |
...
- ラベル: `pillar:sakanaction` / `pillar:datalab` / `pillar:classic` / `ops` / `research`
...
| 炎上 | 他クリエイター比較・批評はしない。データ公開は自分の数値のみ |
