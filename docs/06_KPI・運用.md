...
### 毎日3投稿（12本/日）とコメント返信ルールの両立（#168）

「投稿直後30〜60分は手動でコメント返信」を12本に対して12回やるのは不可能なので、
**公開の時刻を1日1スロットに束ねて、返信ウィンドウを1回にする**:

1. 17:00 `daily_run.py` が12本を生成し**限定公開**で投稿する（自動）
2. 18:00 本人が中身を確認し、`publish_youtube.py --release` で**まとめて公開**（手動・5分）
3. 18:00〜19:00 が唯一の返信ウィンドウ。**全チャンネルのコメントをこの1時間で返す**

視聴者から見た「投稿時刻」は公開切り替えの瞬間なので、アルゴリズム上の初速も
このスロットに集中する。返信は投稿本数ではなく**コメント数**に比例するため、
初期（コメント1桁/本）のうちは1時間で足りる。溢れ始めたら勝ちなので、そのとき見直す。
