...
_SCORE_SYSTEM = """あなたは「ロンロンの天秤」型のショート動画にするネタを選ぶ編集者。
このチャンネルの面白さは**前提を1個だけ壊し、当事者が最後まで大真面目**という分裂にある。
知性は、感情ではなく**言葉かロジックのレベルで裏切りが起きる**ことから生まれる。

ネタの単位はスレ丸ごととは限らない。ベンチマークの直近実測（docs/02 2-5章）では
勢いスレの丸ごと動画化より一発ネタ・寸劇の種が主力だった。
**スレの中のレス1個・一言だけでも、前提の壊れ方が良ければ高く買う。**
...
- 一発ネタ: レス1個・一言で前提が壊れる（音会わせ・言い換えの妙を含む）。短くても減点しない
- 擬人化・設定コント: 物・動物・制度が当事者として大真面目に振る舞う設定が1個立っている
- あるある寸劇の種: 誰もが知る状況1個に、具体的なズレが1個だけある
...
kind には最も近い型を1つ選ぶ:
- tenbin: スレ全体の会話で前提が壊れていく（従来の天秤型）
- hitokoto: レス1個・一言がネタの本体（短尺向き）
- gijinka: 擬人化・設定コント
- aruaru: あるある寸劇の種

採点の目盛り（**絶対評価**。同じバッチに何が並んでいても、この目盛りで付ける。
バッチ内の比較で上下させない）:
- 70点級の実例: 「店員」を「定員」と書き通しレス民が乗っていくスレ／
  漢字を禁止して「みんなこどもにかえろう」と大真面目に呼びかけるスレ
- 50点台: 前提は1個壊れていて型には乗るが、会話の広がりやオチが一段弱い
- 30点台以下: 型に乗らない（雑談・羅列・態度だけ）
- 制度ハックとロジックの裏切りは、**会話が結論に至らず空転しても**、
  問いの立て方そのものが壊れていれば型に乗る（60点級）。オチの有無より前提の質で付ける

各ネタに0〜100点を付ける。**60点以上を付けるのは、上の型に本当に乗るネタだけ**。
...
def score_candidates(threads: list) -> int:
        ...
        output_config={"format": {"type": "json_schema", "schema": {
            ...
            "properties": {"scores": {"type": "array", "items": {
                ...
                "properties": {
                    ...
                    "kind": {"type": "string",
                             "enum": ["tenbin", "hitokoto", "gijinka", "aruaru"],
                             "description": "最も近い型"},
                },
                "required": ["index", "score", "premise", "reason", "kind"],
    ...
    for row in rows:
        ...
        # 型（#295・docs/02 2-5章E）。日次の選定で型をばらけさせるのに使う
        data["ronron_kind"] = row["kind"]
        _save(data)
        scored += 1
    return scored
...
KAKO_QUERIES = (
    ...
    "ガチで悩んでる",
    "俺だけ",
    # ベンチの主力は一発ネタ・あるある・擬人化（#295・docs/02 2-5章E）。
    # その種が集まるスレタイの定型を足す
    "選手権",
    "あるある",
    "ワロタ",
