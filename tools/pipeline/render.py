...
# ズームは固定し、等速のパンだけにする。ズームの変化は画面の拡大縮小として
# 知覚されてチカチカする（本人指摘 2026-08-06、#177）。
# (ズーム, x開始, x終了, y開始, y終了)。位置は取り得る範囲に対する 0〜1 の割合
MOTIONS = [
    (1.14, 0.35, 0.62, 0.50, 0.50),  # 右へ
    (1.14, 0.50, 0.50, 0.34, 0.62),  # 下へ
    (1.14, 0.65, 0.38, 0.50, 0.50),  # 左へ
    (1.14, 0.50, 0.50, 0.66, 0.38),  # 上へ
]
...
def _ken_burns(index: int, dur: float) -> str:
    """シーン番号で向きを変える等速パンを組み立てる（ズームは固定。#177）。

    zoompan の x/y は切り出し矩形の左上を入力座標で指すので、可動域 (iw - iw/zoom) に
    割合を掛ける。ズーム固定でも位置が動けば端の流れで動きは知覚される。
    """
    z, x0, x1, y0, y1 = MOTIONS[index % len(MOTIONS)]
