...
**擬音は1本に必ず1回以上、独立した台詞として鳴らします（無い台本は差し戻されます）。**
音源の効果音は使わないので、**声で鳴らす擬音がこの動画で唯一の効果音**です。
  ○ ず「ガチャ」／たけひろ「ルルルル……ガチャ」／ず「どすん」／こたろう「ポチ」
  ✗「ガチャという音がしたのだ」「電話が鳴ったのだ」
    ← これは音の**説明**です。説明せずに、その音を出してください。
擬音はマーカー（`[叫]` 等）ではなく、台詞の本文として書きます。"""
...
7. **効果音は擬音の台詞として発声する**（「ルルルル」「ガチャ」「ポチ」「どすん」）。
   実測8本すべてに入っていた型で、素人劇の作法が人間味になります。
   音の説明（「ガチャという音がした」）ではなく、**独立した1つの台詞**として鳴らすこと。
...
- シーンは全部で{count + 2}個にします:
  - **シーン1 = 導入**。「◯◯年生まれ位が懐かしい△△{count}選」と宣言する（掛け合い2〜3行）。
    宣言の前に**後輩役のリアクションを1行置いて開幕する**
    （「は？　これ全部絶滅したの？」のように、驚いた後から始める）
  - **シーン2〜{count + 1} = 各ネタ**。1シーン1ネタ。**セリフは4行（2往復）**。
    名称と数字を出したあと、**その1件について会話を往復させる**。
    「名称→反応」で終わらせると一問一答になり、掛け合いになりません（#131）。

    ○ 良い例（2往復。先輩役／後輩役はチャンネルの配役で読み替える）:
      先輩「たまごっち。1年経たずに1000万個売れたのよ」
      後輩「1000万人が世話してたの？　絶対に何割か死んでるじゃん」
      先輩「死んだら墓が出るの。それも含めて流行ったのよ」
      後輩「墓!?　おもちゃに墓を出す会社の気がしれないって」

    ✗ 悪い例（1往復で次へ）:
      先輩「たまごっち。1年経たずに1000万個」
      後輩「え、1000万!?」
...
このチャンネルの作り方（昭和の熱血ビジネスマン。金太郎/半沢型・#167）:
参照はサラリーマン金太郎と半沢直樹です。**時代そのものがイカれていた**ことを描きます。
- **誇張をしない。** 実話の熱量をそのまま出せば、令和から見れば十分に狂っています。
  盛った瞬間にフィクションになり、「実話なのにこれ」の強度が消えます。
- **昭和の常識を、登場人物の全員が大真面目に運用している**のが核です。
  熱血側だけが異常なのではなく、周囲も「そういうものだ」として粛々と動く。
  作中の誰かが時代を疑い始めると、ただの現代ドラマになって壊れます。
  時代を疑う役は**ずんだもん（令和の視点）だけ**に許されます。
- **対決の構図を1つ置く**（部下vs社長、現場vs本社、職人vs制度）。緊張→衝突→啖呵の順。
- **見せ場は啖呵**。「いいか」「やってみせろ」級の言い切りを、与えられた実話の範囲で
  クライマックスに1回だけ作り、**その行を powerword にする**。
  啖呵は短いほど強い（20字以内）。理屈は啖呵の前に済ませておく。
- 熱血側は**叫ぶ**。理屈より現場、根性で押し切る。
  ただし**憎めない**ように書く（部下想いの一面や、後で自分が折れる場面を入れる）。
- ずんだもんは**令和の視点で眩暈を起こす**。「今なら一発アウトなのだ」が担当。
...
- 平成を生きた先輩役が懐かしがり、平成を知らない後輩役が現代の目線でツッコむ構図にする。
  これで同世代以外にも「そんな時代があったのか」で成立させる。
  **後輩役のツッコミは「精密な例え」で**（「それ暗号じゃん。読める方がおかしいって」）。
...
# 決まり文句の辞書だけでは「飯と仕事は別なのだ」（断定）や「作れるか」（反語）を
# 拾えず検知漏れになるため、語彙（評価語）と構文（反語・仮定・比較・断定）の両面で見る（#153）
_OPINION = re.compile(
    # 一人称・二人称で自分の考えを言う決まり文句
    r"(ボクなら|わたくしなら|わたしなら|と思う|わたくしは|ボクは|あなたは|お前は|キミは|"
    # 好悪・願望。「認めたくない」「ほしい」は判断そのもの
    r"嫌|好き|やだ|いらな|ほしい|たくない|"
    # 当為・確信の言い切り
    r"べき|はず|でしょ|だろ|絶対|に決まって|間違いない|違いない|言い切|言える|"
    # 評価・断定の語彙。「異常なのだ」「怖いのだ」は感想ではなく判断
    r"ありえな|信じ|許せ|羨ま|ずるい|かわいそう|無理|異常|怖い|おかしい|ひどい|"
    r"すごすぎ|やばい|草|意味が|気がしれ|ヒマ|地獄|損|得|妥協|飛躍|甘え|"
    # 反語。「作れるか」「食えるか」は質問ではなく強い断定
    r"(?:れる|える|できる)か(?:[！？!?。…]|$)|"
    # 仮定を置いて判断する形。「今なら『演出』で終わる」
    r"今なら|昔なら|普通なら|だったら|"
    # 比較の判断。「現場で食う方がうまい」
    r"(?:方|ほう)が|"
    # 断定コピュラ「AはBなのだ」。数字入りは事実の提示が多いので拾わない
    r"は[^、。！？\d]{1,10}なのだ(?:[。…]|$))"
)
...
# 声で鳴らす擬音の判定（#245）。ロンロン実測8本すべてに入っていた型で、
# 「効果音は基本使わない・音の演出は声で」という本人方針と噛み合う**唯一の効果音**。
# プロンプトに「発声してよい」と書いてあったが、実出力は台本55本中1本だった。
#
# 判定は**独立した塊になっていること**を見る。「ガチャと鳴った」は音の説明であって
# 発声ではないので、区切りの中に文が入っているものは（漢字が混ざるか長さで）落ちる。
#
# 種は**語幹だけ**をひらがなで持ち、カタカナはひらがなに寄せてから照合する
# （擬音はどちらでも書かれる。表記ごとに二重に並べると必ず片方を足し忘れる）。
# 語幹の後ろに付けてよいのは音を伸ばす・詰める文字だけなので、
# 「ばん」は「ばーん」「ばんっ」を拾うが「ばんだい」は拾わない
_ONOMATOPOEIA_SEEDS = (
    "がちゃ", "かちゃ", "かち", "ぽち", "ぱち", "ぴしゃ", "ぱか", "こつ", "ざわ",
    "どき", "ごご", "じり", "ぷる", "るる", "ぶぶ", "ぴー", "ぶー", "ちー", "しー",
    "どすん", "どん", "ばたん", "ばん", "ずどん", "ごん", "とん", "ぴんぽん",
    "ごと", "がこ", "ばき", "ぱた", "かた", "ごつ", "がつ", "どか", "ぼこ",
    "がた", "ぐら", "ごろ", "ぱら", "ひら", "ぎゅ", "きゅ", "ぷし", "しゅ", "ずる",
    "ばた", "ぴん", "ぽん", "ばさ", "べた", "ぴた", "ぬる", "むに", "ざー", "しー",
)
_ONOMATOPOEIA_SEED = re.compile(
    "^(?:" + "|".join(sorted(_ONOMATOPOEIA_SEEDS, key=len, reverse=True))
    + r")[ーっんぁぃぅぇぉ〜]*$"
)
# 台詞の中の「独立した塊」の区切り。読点・感嘆符・引用符と、割り込み記号で切る
_SEGMENTS = re.compile(r"[、。，．！？!?…‥・「」『』\s]+|" + INTERRUPT_MARK)
_KANA_ONLY = re.compile(r"^[ぁ-んァ-ヴーｰ〜]+$")
_KATAKANA_ONLY = re.compile(r"^[ァ-ヴーｰ〜]+$")
# 「ルルルル」「ンニィィィィィ」型。3連以上でないと「なのだーーー」まで拾ってしまう
_REPEAT_CHAR = re.compile(r"(.)\1{2,}")
# 「ドキドキ」「ガチャガチャ」型。2〜3文字の単位が丸ごと繰り返される形
_REPEAT_UNIT = re.compile(r"^(.{2,3})\1+[ーッっ〜]*$")


def _to_hiragana(text: str) -> str:
    """カタカナをひらがなに寄せる。擬音はどちらの表記でも書かれるため。"""
    return "".join(chr(ord(c) - 0x60) if "ァ" <= c <= "ヴ" else c for c in text)


def _is_onomatopoeia(seg: str) -> bool:
    """この塊が「声で鳴らした効果音」に見えるか。

    **緩く見るより厳しく見る。** 見逃せば「擬音が無い」と差し戻すだけで済むが、
    通しすぎると検査そのものが黙って効かなくなる（プロンプトに書いただけの状態に戻る）。
    そのため撥音止め（「〜ン」）は形では認めず、種リストに載っているものだけ通す。
    「ジーパン」「バンダイ」のような外来語が擬音として通ってしまうため。

    形で認めるのは、外来語にはまず現れない次の2つだけ:
    - 同じ文字の3連以上（「ルルルル」「ンニィィィィィ」）
    - 2文字単位の繰り返し（「ドキドキ」「ガチャガチャ」）
    どちらもカタカナ表記のときに限る。ひらがなの短い塊は相槌・感嘆
    （「うわあ」「はいはい」「なのだーーー」）が圧倒的に多く、形では区別できない。
    """
    if not (2 <= len(seg) <= 12) or not _KANA_ONLY.match(seg):
        return False
    if _ONOMATOPOEIA_SEED.match(_to_hiragana(seg)):
        return True
    if not _KATAKANA_ONLY.match(seg):
        return False
    return bool(_REPEAT_CHAR.search(seg) or _REPEAT_UNIT.match(seg))


def has_onomatopoeia(lines: list) -> bool:
    """台詞のどこかで擬音を声に出しているか。"""
    return any(_is_onomatopoeia(seg.strip())
...
def form_issues(script: dict, cfg: dict | None = None) -> list:
    ...
    # 鳴らすものが無い。ロンロン型の判定より前に置くのは、showa が
    # ロンロン型ではない（金太郎/半沢型・#167）が寸劇ではあるため
    if (cfg.get("style") or {}).get("skit") and not has_onomatopoeia(lines):
        issues.append(
            "擬音を声で鳴らしている台詞がありません。"
            "効果音は音源ではなく声で作るので、"
...
def dialogue_issues(script: dict, cfg: dict | None = None) -> list:
    ...
    # 同じ行の繰り返しは1回だけ数え、言い回しが1種類しか無い台本も薄いとみなす
    # （決まり文句を並べるだけで通る抜け道を塞ぐ。#153）
