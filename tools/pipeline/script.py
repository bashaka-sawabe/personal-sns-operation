...
キャラは**スレの外から報告する解説者ではなく、スレの中の登場人物**です。
その場で起きていることとして演じてください。

- **ずんだもんがスレ主**（相談する人・自慢する人・困っている人）を演じます。
- **他のキャラがレス民**を演じます。1人1レスではなく、性格を持った数人が絡みます。
- 声は登場人物ごとに変えます（使える話者は speaker の候補にあるものだけ）。
...
**代わりに、その人に喋らせる:**
  ✗ め「そこへ『着ないなら離婚と脅せ』と助言が来たわ」
  ○ たけひろ「スカート履かないなら離婚だ、って脅してみろよ」
     ずんだもん「待つのだ、なんでそうなるのだ!?」
...
**尺（守れないと動画として成立しません）:**
この動画は**26〜32秒**に収めます。話速から逆算すると、
**セリフの合計は230字が上限**です（15行前後）。**これは超えられません。**
1行増やすなら、どこかを削ってください。

その230字の中で、**行の長さは揃えないでください**。
差し込みは5〜15字、スレ主が言い訳や自分語りをする場面は40字まで伸ばしてよい。
**全員が同じ長さの行を交互に出すと、一言コメント係の集まりになります。**
説明的な行を入れない。

**出力する前に、この5つを数えて確認してください:**
1. セリフの合計字数は **230字以下** か
2. 行末が `──` で終わり、次を別の人が奪う箇所が **2箇所以上** あるか
3. 同じ人が続けて2行喋る箇所が **1箇所以上** あるか
4. パワーワードが指定どおりの形で最後のシーンに **1回** 入っているか
5. 質問で終わる行が **3行以下** か
...
**同じ人が続けて2行以上喋る箇所を、1本に最低1回**作ってください。
